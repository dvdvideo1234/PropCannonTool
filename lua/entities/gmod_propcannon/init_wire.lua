local function wireError(oE, sM)
...
local function wireSetupPorts(oE, sF, tI, bL)
  ...
  while(tI[iD]) do local sN, sT, sD = wireUnpackPortInfo(tI[iD])
    if(not sN) then wireError(oE, "("..sF..")["..iD.."]: Name missing"); return oE end
    if(not sT) then wireError(oE, "("..sF..")["..iD.."]: Type missing"); return oE end
    if(not WireLib.DT[sT]) then wireError(oE, "("..sF..")["..iD.."]: Type invalid ["..sT.."]"); return oE end
  ...
  if(bL) then
    for iD = 1, #tN do -- Port name and type is mandatory
      ...
      if(not bS) then wireError(oE, "("..sF..")["..iD.."]: Error: "..sE); return oE end
